# The original pipeline, as built

This is the manual for the system the README's findings are about: the 20-source
dataset builder, the ConvNeXt/ResNet training harness, its evaluation and
Grad-CAM tooling. It was the body of the README until 2026-09-13 and is kept
here because the code still exists and still runs.

Read it knowing what `README.md` and `LIMITATIONS.md` established: the models
this pipeline produced score 89 percent by recognising which source corpus a
file came from, not by reading image content. Nothing below has been rewritten
to hide that, and the correction notes inserted during the audit are left where
they were. For the model that replaced it, see the README's "The rebuild".
